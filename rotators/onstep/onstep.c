...
/**
 * Command list:
 * See https://www.onstep.com/support/LX200CommandSet.pdf
 * and https://www.onstep.com/support/TelescopeProtocol_2010-10.pdf for newer
 * Firmware Versions
 *
 * Not the full set of available commands is used, the list here shows
 * only the commands of the telescope used by hamlib
 *
 * All used Commands are supported by the Onstep Telescope controller
 * with enhanced LX200 commands.
 *
 ...
 */
static int onstep_transaction(ROT *rot, const char *cmdstr,
 ...
 * Initialization
 */
static int onstep_init(ROT *rot)
{
    struct onstep_priv_data *priv;

    rot->state.priv = (struct onstep_priv_data *)
                      calloc(1, sizeof(struct onstep_priv_data));
 ...
 * Cleanup
 */
static int onstep_cleanup(ROT *rot)
{
 ...
 */
static int onstep_open(ROT *rot)
{
    char return_str[BUFSIZE];
    size_t return_str_size = 0;
    struct onstep_priv_data *priv = (struct onstep_priv_data *)rot->state.priv;
    int retval;
    ...
    retval = onstep_transaction(rot, ":GVP#", return_str, &return_str_size,
                               sizeof(return_str));

    if (retval != RIG_OK) { rig_debug(RIG_DEBUG_ERR, "%s: onstep_transaction %s\n", __func__, rigerror(retval)); }
    ...
    {
        retval = onstep_transaction(rot, ":AL#:So00#:Sh90#", NULL, 0, 0);
    }
    else
    {
        // Audiostar elevation is in arcminutes
        retval = onstep_transaction(rot, ":So00#:Sh5400#", NULL, 0, 0);
    }
 ...
 */
static int onstep_close(ROT *rot)
{
    rig_debug(RIG_DEBUG_VERBOSE, "%s called\n", __func__);
    /* Stop all Movement */
    return onstep_transaction(rot, ":Q#", NULL, 0, 0);
}
 ...
 * el: Target elevation
 */
static int onstep_set_position(ROT *rot, azimuth_t az, elevation_t el)
{
    struct onstep_priv_data *priv = (struct onstep_priv_data *)rot->state.priv;
    ...
    onstep_transaction(rot, ":D#", return_str, &return_str_size, sizeof(return_str));
    ...
    onstep_transaction(rot, cmd_str, return_str, &return_str_size, 3);
 ...
 */
static int onstep_get_position(ROT *rot, azimuth_t *az, elevation_t *el)
{
    char return_str[BUFSIZE];
    char eom;
    size_t return_str_size;
    ...
    onstep_transaction(rot, ":GZ#:GA#", return_str, &return_str_size, BUFSIZE);
 ...
 * Stops all movement
 */
static int onstep_stop(ROT *rot)
{
    struct onstep_priv_data *priv = (struct onstep_priv_data *)rot->state.priv;
    azimuth_t az;
    elevation_t el;
    ...
    onstep_transaction(rot, ":Q#", NULL, 0, 0);
    onstep_get_position(rot, &az, &el);
 ...
 */
static int onstep_park(ROT *rot)
{
    rig_debug(RIG_DEBUG_VERBOSE, "%s called\n", __func__);

    /* Assume home is 0,0 */
    onstep_set_position(rot, 0, 0);
 ...
 */
static int onstep_reset(ROT *rot, rot_reset_t reset)
{
    rig_debug(RIG_DEBUG_VERBOSE, "%s called\n", __func__);
    onstep_park(rot);
 ...
 */
static int onstep_move(ROT *rot, int direction, int speed)
{
    struct onstep_priv_data *priv = (struct onstep_priv_data *)rot->state.priv;
    ...
    {
    case ROT_MOVE_UP:
        return onstep_set_position(rot, priv->target_az, 90);

    case ROT_MOVE_DOWN:
        return onstep_set_position(rot, priv->target_az, 0);

    case ROT_MOVE_CCW:
        return onstep_set_position(rot, -180, priv->target_el);
...
static const char *onstep_get_info(ROT *rot)
{
    static char buf[256]; // this is not thread-safe but not important either
    struct onstep_priv_data *priv = (struct onstep_priv_data *)rot->state.priv;
    rig_debug(RIG_DEBUG_VERBOSE, "%s called\n", __func__);

    sprintf(buf, "Onstep telescope controller with enhanced LX200 protocol.\nModel: %s",
            priv->product_name);
    return buf;
}
...
const struct rot_caps onstep_caps =
{
    ROT_MODEL(ROT_MODEL_ONSTEP),
    .model_name =       "Onstep",
    .mfg_name =         "Onstep",
    .version =          "20210222.0",
    .copyright =        "LGPL",
    ...
    .rot_init =         onstep_init,
    .rot_cleanup =      onstep_cleanup,
    .rot_open =         onstep_open,
    .rot_close =        onstep_close,

    .set_position =     onstep_set_position,
    .get_position =     onstep_get_position,
    .park =             onstep_park,
    .stop =             onstep_stop,
    .reset =            onstep_reset,
    .move =             onstep_move,

    .get_info =         onstep_get_info,

    .get_conf =         rot_get_conf,
    .set_conf =         rot_set_conf,
};

DECLARE_INITROT_BACKEND(onstep)
{
    rig_debug(RIG_DEBUG_VERBOSE, "%s: _init called\n", __func__);

    rot_register(&onstep_caps);
